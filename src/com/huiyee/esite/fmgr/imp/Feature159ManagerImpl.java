...
import com.huiyee.esite.dto.Feature159Dto;
import com.huiyee.esite.dto.IDto;
import com.huiyee.esite.dto.showdto.Show154Dto;
import com.huiyee.esite.fdao.IHd159Dao;
...
public class Feature159ManagerImpl extends AbstractFeatureManager{
	private IHd159Dao hd159Dao;
	private IPageDao pageDao;
	private ITemplateDao templateDao;
	private IContentCategoryDao contentCategoryDao;
	
	public void setHd159Dao(IHd159Dao hd159Dao)
	{
		this.hd159Dao = hd159Dao;
	}
	...
	public long add(long pageid, long featureid, String featurename) {
		long fid = hd159Dao.saveFeatureInteract(pageid);
		return getPageFeatureDao().addPageFeature(pageid, featureid, fid,featurename,"N");
	}

	@Override
	public IDto config(long fid, Account account) {
		Feature159Dto dto = new Feature159Dto();
		HD154Model m = hd159Dao.findForumidListByFid(fid);
		if(m.getIds() != null){
		...
		List<ContentCategory> list=hd159Dao.findCategory(account.getOwner().getId());
	...
	public String configSub(long featureid, IDto dto, Account account) {
		Feature159Dto d = (Feature159Dto)dto;
	    try
		{
			if (d.getList()!=null && d.getList().size() > 0)
			{
				...
				hd159Dao.saveFid(d.getFid(),ids , type,d.getPageid());
				for(Long catid : d.getList()){
					hd159Dao.updateContentCategoryPageid(catid, d.getPageid());//记录该目录的最近绑定在哪个page上
				}
			}
			else
			{
	...
	public String configSubNew(long featureid, IDto dto, Account account) {
		...
		if(page != null){
			...
			if(d.getList()!=null){
				for(Long catid : d.getList()){
					hd159Dao.updateContentCategoryPageid(catid, page.getId());
				}
			}
		}
		
		PageBlockRelation pbr = hd159Dao.findPageBlockRelationByRelationid(d.getRelationid());
		JSONObject jo1 = JSONObject.fromObject(pbr.getJson());
		jo1.element("obj", jo.toString());
		hd159Dao.updatePageBlockRelationByRelationid(d.getRelationid(),jo1.toString());
		return "Y";
	}
}
