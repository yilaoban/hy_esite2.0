...
public class DataUserAction extends AbstractAction {
	...
	public String user() throws Exception {
		...
		SolrDataDto sdd = hylogSolrServer.dataUser(ownerid, mtype, 0, 0, start, rows, sDate, eDate);
	...
	public String userDetail() throws Exception {
		...
		SolrDataDto sdd = hylogSolrServer.dataUserDetail(ownerid, mtype, mid, site, page, start, rows);
		int total = sdd.getTotal();
		SolrDocumentList list = sdd.getDocumentList();
		...
		long preCreated = System.currentTimeMillis();
		List<SolrDocument> ulist = new ArrayList<SolrDocument>();
		for (int i = 0; i < list.size(); i++) {
			SolrDocument doc = list.get(i);
			...
			String vl = "--";
			long dd = preCreated - cal.getTimeInMillis();
			if (dd < 30 * 60 * 1000) {
				vl = dd / 1000 + " s";
			}
			doc.addField("visitlong", vl);
			preCreated = cal.getTimeInMillis();

			String url = (String) doc.getFieldValue("url");
			doc.addField("visitpage", HyConfig.getPageyuming(this.getOwner()) + url);
