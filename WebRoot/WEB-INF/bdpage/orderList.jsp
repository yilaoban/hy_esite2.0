...
<div class="top">
	<a href="javascript:void(0);" onclick="window.history.back()" class="left"><img src="/images/shop/back.png" />&nbsp;返回</a>我的订单<s:if test='status == "DFK"'>（待付款）</s:if><s:else>（全部）</s:else>
	<a href="#" class="right"></a>
</div>
<div class="nav">
        <ul>
        <li <s:if test="status == null">class="one red"</s:if><s:else>class="one"</s:else>><a class="mine" href="/${oname}/user/showOrderList.action">全部</a></li>
        <li <s:if test='status == "DFK"'>class="two red"</s:if><s:else>class="two"</s:else>><a class="mine"  href="/${oname}/user/showOrderList.action?status=DFK"><s:if test='status != "DFK" && dto.dfkCount > 0'><i>${dto.dfkCount }</i></s:if>待付款</a></li>
        <li class="two"><a href="/${oname}/user/showKqOrderList.action" class="mine" ><s:if test='dto.kqCount > 0'><i>${dto.kqCount }</i></s:if>我的卡券</a></li>
        <li class="last"><a href="/${oname}/user/myAddress.action?type=W" class="mine" >我的地址</a></li>
        </ul>
</div>
...
<div class="main" id="wrapper">
	<ul id="cont">
		...
		<li id="order_list${d.id }">
              ...
              <p class="total"><span>${d.createtimeStr}</span>共<i>${d.goodscount}</i>件商品&nbsp;&nbsp;实付款：<i>￥${d.realprice/100 }</i></p>
              <p class="second">
		 		...
		 		<s:if test="#d.expressid != null">
		 			<a href="javascript:void(0);" onclick="showexpress('${d.expressid}')" class="use">物流单号</a>
		 		</s:if>
              </p>
        </li>
        </s:iterator>
...
<script>
        ...
        function load1() {
             	$.post("/${oname}/user/ajaxShowOrderList.action",{"pageId":generatedCount,"status":'${status}'},function(data){
		    		if(data){
		    			if(data.payOrderList){
							$.each( data.payOrderList, function(i, n){
								...
								$.each(n.goods,function(i2,n2){
              					...
              					html += '</ul>';
              					html += '<p class="total"><span>'+n.createtimeStr+'</span>共<i>'+n.goodscount+'</i>件商品&nbsp;&nbsp;实付款：<i>￥'+n.realprice/100+'</i></p>';
              					html += '<p class="second">';
              					if(n.status == "DFK"){
              					...
              					if(n.expressid){
	              					html += '<a href="javascript:void(0);" onclick="showexpress(\''+n.expressid+'\')" class="use">查看物流单号</a>';
              					}
              					html += '</li>';
              					html += '</p>';
								$("#cont").append(html);
