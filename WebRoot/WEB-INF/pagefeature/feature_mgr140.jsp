...
<form  id="myform">
	...
	请选择竞拍的选项：<br/>
	  <s:iterator value="dto.journalList" var="apt">
           <label class="forradio"><input type="radio" value="${apt.id}" name="dto.jid" <s:if test="dto.jid == #apt.id"> checked="checked" </s:if> /> ${apt.title} </label><br />
      </s:iterator>
	<input type="button" id="save140" value="确定"/>
	<input type="button" value="取消" onclick="closeFrame()"/>
</form>
<script type="text/javascript">
		$("#save140").click(function(){
			$.ajax({
		        cache: true,
		        type: "POST",
