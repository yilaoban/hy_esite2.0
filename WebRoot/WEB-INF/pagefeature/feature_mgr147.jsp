...
<form  id="myform">
	...
	<input type="hidden" name="dto.fid" value="${fid}" />
	  <s:iterator value="dto.catList" var="cat">
           <label class="forradio"><input type="radio" value="${cat.id}" name="dto.catid" <s:if test="dto.catid == #cat.id"> checked="checked" </s:if> /> ${cat.name} </label><br />
      </s:iterator>
      详情页面:
	...
	<input type="button" id="save147" value="确定"/>
	<input type="button" value="取消" onclick="closeFrame()"/>
</form>
<script type="text/javascript">
		$("#save147").click(function(){
			$.ajax({
		        cache: true,
		        type: "POST",
