...
<script type="text/javascript">
	function formsub(){
		...
		var datedays = $("input[name='yuYueSSTime.datedays']");
	   ...
	   for(var j=0; j<datedays.length; j++){
	   	if(datedays[j].value == ""){
		 	$("#datedays_").html("日期不能为空!").css("color", "RED");
			return;
		 } 
	   }
		$.ajax({
	        cache: true,
	        type: "POST",
	        url:'/${oname }/interact/saveYuYueDaySSTime.action',
...
<a name="maodian"></a>
<div>
  <form action="/${oname }/interact/saveYuYueDaySSTime.action" method="post" enctype="multipart/form-data" id="form1" class="formview jNice">
  <input type="hidden" value="${catid }" name="catid" />
   ...
   <input type="hidden" value="1" name="yuYueSSTime.type" />
	...
	<dl>
	 	<dt>选择日期</dt>
		<dd>
			 <input type="text" value="${dateday}" name="yuYueSSTime.datedays" class="Wdate" onFocus="WdatePicker({dateFmt:'yyyyMMdd'})" readonly="readonly" />
			 <span id="datedays_"></span>
			<a id="addriqi" href="javascript:void(0);" onclick="addriqi()"><b>+</b>更多</a>
		</dd>
	</dl>
  ...
  <script type="text/javascript">
	...
	function addriqi(){
		var html = '<br/><input type="text" name="yuYueSSTime.datedays" class="Wdate" onFocus="WdatePicker({dateFmt:\'yyyyMMdd\'})" readonly="readonly" />';
		$('#addriqi').before(html);
	}
