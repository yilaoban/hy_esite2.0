...
<html>
<head>
		<s:if test='page.bgJson!=null && page.bgJson.title!=""'>
	...
	<body style="<s:if test='dto.pc.size > 1'>overflow:hidden;</s:if><s:if test='page.bgJson!=null && page.bgJson.background !=null && page.bgJson.background !=""'>background:${page.bgJson.background} repeat;</s:if>">
	...
	<%--  <div id="loading" class="loading"><img src="images/load.gif"/>加载中...</div> --%>
		...
		<s:iterator value="dto.pc" var="c" status="st">
			...
			<script type="text/javascript">
				...
				if(id > 0){
					if('${oname}' !=''){
						$("#a_"+id).load('/${oname}/user/showCard.action?pcid='+id+"&pageid="+'${pageid}'+'&jspstyle='+'${page.jspstyle}'+"&css="+'${c.css}',function(responseTxt,statusTxt,xhr){
						    if(statusTxt=="success")
						    	var maxH = 0;
						    	$(".block, .hyzj").each(function(){
						    	    bH = $(this).height();
						    	    bT = $(this).position().top;
						    	    var aH = bH + bT;
						    	    if(aH){
						    	         if(aH >maxH){
						    	        	 maxH = aH;
						...
						$("#a_"+id).load('/user/showCard.action?pcid='+id+"&pageid="+'${pageid}'+'&jspstyle='+'${page.jspstyle}'+"&css="+'${c.css}',function(responseTxt,statusTxt,xhr){
						    if(statusTxt=="success")
						    	var maxH = 0;
						    	$(".block, .hyzj").each(function(){
						    	    bH = $(this).height();
		...
		<input id="hy_kv" type="hidden" value="${kv}">
		</div>
		<script type="text/javascript">
			(function(){
