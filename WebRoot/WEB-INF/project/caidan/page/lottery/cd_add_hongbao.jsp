...
<script type="text/javascript">
	...
	function checkSub(){
		if($("#title").val()==""){
			...
			$('#startnote').val("抢红包活动还未开始");
			}
			if($('#endnote').val().trim() == ""){
				$('#endnote').val("抢红包活动已经结束");
			}
			var enid = $('#enid').val();
			if(enid == 0){
...
<div class="wrap_content left_module">
  <form action="save_hongbao_sub.action" method="post" enctype="multipart/form-data" id="form1" class="formview jNice">
  	<input type="hidden" name="htype" value="H" />
  	<div class="toolbar pb10">
  	  <ul class="c_switch">
	  <li class="selected"><a href="">新增抢红包</a></li>
	  </ul>
	  <a href="/${oname }/cd-lottery/index.action?htype=H" class="return" title="返回"></a>
	</div>
	<dl>
	 	<dt>标题</dt>
		<dd>
	...
	<dl>
	 	<dt>开始前提示语</dt>
		<dd>
			<input type="text" class="text-long" name="startnote" placeholder="抢红包活动还未开始" id="startnote"><span class="notice">提示用户该活动还没开始</span>
		</dd>
	</dl>
	<dl>
	 	<dt>结束后提示语</dt>
		<dd>
			<input type="text" class="text-long" name="endnote" placeholder="抢红包活动已经结束" id="endnote"><span class="notice">提示用户该活动已结束</span>
		</dd>
	</dl>
	<dl id="ruletype">
	 	<dt>抽奖资格</dt>
