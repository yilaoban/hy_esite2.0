...
<html>
	<head>
		...
		#banner {background:#167EA6;}
		#bottom {margin-top:20px;}
        #foot {margin-top:20px;background:#fff;border-top:1px solid #EEEEEE;text-align:center;padding:15px 0;font-size:14px;}
        #loginbox {  position: absolute;top: 50px;left:50%;margin-left:200px;font-size:14px;color:#888;background:#fff;border-radius:4px;}
    ...
    <body>
	    <div id="hd">
	    	<div class="main-wrap">
				<div id="logo"><img src="/project/caidan/img/logo.jpg" style="width:180px;"></div>
		    </div>
		</div>
		<div id="banner" style="position:relative;">
				<div class="slider">
					<div class="frame" style="height:402px;background:url(/images/1.jpg) center center;"></div>
				</div>
			        <div id="loginbox">      
			        	<div class="loginform">
			        		<h3 style="margin:10px 0;">请登录</h3>
				            <s:form method="post" action="sign" id="myform">
							<div class="inputdiv" style="background:url(/project/caidan/img/id.png) no-repeat 12px 14px;">
								<input type="text" placeholder="用户名"  name="accountname"  value="${accountName}" id="accountloginName"/>
							</div>
							<div class="inputdiv" style="background:url(/project/caidan/img/pwd.png) no-repeat 14px 10px;border-bottom:1px solid #eaeaed;">
								<input type="password" placeholder="密码"  name="password" id="loginpassword"/>
							</div>
							<div class="inputdiv" style="background:url(/project/caidan/img/mmb.png) no-repeat 12px 10px;">
								<input type="text" placeholder="验证码" name="checkCode" id="checkCode"/>
								<img class="main_input_box" id="imageC" src="${pageContext.servletContext.contextPath }/checkCode.cc"  style="cursor:pointer;display:block;" onclick="myreload()" title="点击更换验证码"/>
							</div>
