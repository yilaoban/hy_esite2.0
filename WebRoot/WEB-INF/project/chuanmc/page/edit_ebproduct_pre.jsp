...
<form action="edit_ebproduct_sub.action" method="post" enctype="multipart/form-data" id="form1" class="formview">
<input type="hidden" value="${dto.cp.id }" name="contentId"/>
<input type="hidden" value="${dto.cp.catid }" name="ccid"/>
<input type="hidden" value="${dto.cp.type }" name="productType"/>
<span id="errrSpan" style="color:red"><strong style="color:red"><s:fielderror theme="" fieldName="error"/></strong> </span>
  <div class="toolbar pb10">
	...
	<dl>
	 	...
	 	<s:elseif test='dto.cp.type == "J"'>
	 		<dt>原积分</dt>
	 		<dd>
			<input type="text" class="text-long" name="price" value="${dto.cp.jifen }" onkeyup="this.value = this.value.replace(/\D/g,'');">
			</dd>
	 	</s:elseif>
	...
	<dl>
	 	...
	 	<s:elseif test='dto.cp.type == "J"'>
	 		<dt>现在积分</dt>
	 		<dd>
			<input type="text" class="text-long" name="salesprice" value="${dto.cp.salesjifen }" onkeyup="this.value = this.value.replace(/\D/g,'');">
			</dd>
	 	</s:elseif>
	...
	<s:if test='dto.cp.type != "J"'>
		<dl>
		 	<dt>最多抵扣的积分</dt>
			<dd>
				<input type="text" class="text-medium" name="maxjf" value="${dto.cp.maxjf }" onkeyup="this.value = this.value.replace(/\D/g,'');">
			</dd>
		</dl>
	</s:if>
	<dl>
	 	<dt>每人最多购买次数</dt>
		<dd>
			<input type="num" class="text-medium" name="personlimit" value="${dto.cp.personlimit }">（0表示不限制购买）
		</dd>
	</dl>
	<dl>
	 	<dt>产品总数</dt>
		<dd>
			<input type="text" class="text-long" name="total" value="${dto.cp.total }"  onkeyup="this.value = this.value.replace(/\D/g,'');">
		</dd>
	</dl>
	<dl>
	 	<dt>已使用数</dt>
		<dd>
			<input type="text" class="text-long" name="used" value="${dto.cp.used }"  onkeyup="this.value = this.value.replace(/\D/g,'');">
		</dd>
	</dl>
	<dl>
	 	<dt>展示使用数</dt>
	...
	<dl>
	 	<dt>商品类型</dt>
		<dd>
			<label class="forradio"><input type="radio" name="subtype" value="S" <s:if test='dto.cp.subtype=="S"'>checked="checked"</s:if>>物流商品</label>
			<label class="forradio"><input type="radio" name="subtype" value="K" <s:if test='dto.cp.subtype=="K"'>checked="checked"</s:if>>自提商品</label>
		</dd>
	</dl>
	<dl>
	 	<dt>摘要</dt>
