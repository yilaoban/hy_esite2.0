...
<script type="text/javascript">
	...
	var activityid = <s:property value="%{activityid}"/>;
	   function getForm(){  
            jQuery.getJSON('/page/ajax_find_participate_data.action',{"sitegroupid": sitegroupid,"activityid":activityid,"inajax":1}, function(data) { 
	...
	function getForm1(){  
            jQuery.getJSON('/page/ajax_find_participatesource_data.action',{"sitegroupid": sitegroupid,"activityid":activityid,"inajax":1}, function(data) { 
		    var visitTerminal = new Highcharts.Chart({
					        chart: {
					            plotBackgroundColor: null,
					            plotBorderWidth: null,
	...
	<tbody>
		<s:iterator value="dto.terminal" var="s">
			<tr align="center" >
			    ...
			    <td>${s.count}</td>
			    <td>${s.countSum}</td>
			    <th><a href="javascript:void(0)" onclick="activity_details('${s.name }',<s:property value="%{activityid}"/>)">查看详情</a></th>
			</tr>	
		</s:iterator>
	</tbody>
</table>
