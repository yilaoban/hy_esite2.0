...
<link href="/css/shop/shopping cart.css" rel="stylesheet" type="text/css">
	<script type="text/javascript">
	  	function updateQuantity(type,shopCartid,productid){
		  	...
		  	if(quantity > 0){
		  		$.post("/${oname}/user/updateQuantity.action",{"shopCartid":shopCartid,"quantity":quantity,"productid":productid},function(data){
		  			if(data > 0){
...
<div class="main">
	<form action="/${oname}/user/showOrder.action?type=W" method="post" id="shopform">
  	<ul>
  		<s:iterator value="dto.shopCartList" status="st" var="p">
             ...
             <b class="border">
             	...
             	<input type="hidden" name="salesprice" value="${p.product.salesprice }">
             </b>
			...
			<s:else>
             ...
             <p class="tenth"><i>￥${p.product.salesprice }</i><span>￥${p.product.price }</span></p><br>
             <p class="num"><i class="jian" onclick="updateQuantity('S',${p.id},${p.product.id})"><img src="/images/shop/jian.png"></i><span id="quantity_${p.id}">${p.num}</span><i class="jia" onclick="updateQuantity('A',${p.id},${p.product.id })"><img src="/images/shop/jia.png"></i></p>
...
<div class="bottom">
	...
	<a href="javascript:void(0);" onclick="showDingdan()">去结算(<em>0</em>)</a><span>合计: <em>0.00</em>元</span>
</div>
	...
	<div class="bottom2">
<p>
	<a href="/${oname}/user/payHome.action" class="shouye">首页</a><a href="/${oname }/user/payProductList.action" class="fenlei">分类</a><a href="#" class="car2">购物车</a><a href="/${oname}/user/showOrderList.action" class="mine">我的</a>
</p>
</div>