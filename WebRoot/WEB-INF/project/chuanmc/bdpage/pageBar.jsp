...
<script type="text/javascript">
  ...
  var a="pageId=${dto.pager.currentPage}";
  var b="pageId="+pageId;
  var u = "${url1}";
  var url=decodeURI(u);
  if(url.indexOf("?") == -1){
...
<div id="pagination">
	<s:if test="dto.pager.totalPage > 0">
		<s:if test="dto.pager.hasPrevPage">
			...
			<a href="${url}" class="pre" rel="external">上一页</a>
		</s:if>
		<s:if test="dto.pager.totalPage >= 1">
			...
			<s:if test="dto.pager.currentPage == 1">
				<a class="current" href="${url}" rel="external" style="color: red">1</a>
			</s:if>
			<s:else>
				<a href="${url}" rel="external">1</a>
			</s:else>
		</s:if>
		...
		<s:iterator var="pageIndex" value="dto.pager.pageBarList" begin="1" >
			<s:if test="dto.pager.currentPage == #pageIndex">
				<a class="current" style="color: red"><s:property value="pageIndex" /></a>
			</s:if>
			<s:else>
				<s:url var="url" includeParams="all"> 
					<s:param name="pageId" value="pageIndex"></s:param>
				</s:url>
		...
		<s:if test="dto.pager.totalPage >1 && (dto.pager.currentPage +3 <= dto.pager.totalPage)">
			...
			<s:if test="dto.pager.currentPage == dto.pager.totalPage">
				<a class="current" href="${url}" rel="external" style="color: red">${dto.pager.totalPage}</a>
			</s:if>
			<s:elseif test="dto.pager.currentPage +3 < dto.pager.totalPage">
				<a href="${url}" rel="external">${dto.pager.totalPage}</a>
			</s:elseif>
		</s:if>
		<s:if test="dto.pager.hasNextPage">
			...
			<a href="${url}" class="next" rel="external">下一页</a>
		</s:if>
