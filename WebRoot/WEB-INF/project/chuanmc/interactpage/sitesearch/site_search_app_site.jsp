    ...
    <s:iterator value="dto.list" var="s" status="sta">
      <div class="sitegroup_List sg_${s.id}" style="max-height: 150px">
        ...
        <div class="sitegroup_List_left2">
          <div class="title">
            <b>${s.groupname}<s:if test='#s.istemplate=="Y"'>
                <span class="glyphicon glyphicon-star" aria-hidden="true" title="活动模板"></span>
              </s:if>
            </b>
            <a href="javascript:;" onclick="changeSiteName1('${s.id}','${s.groupname}')">
          ...
          <div class="operations">
            <s:iterator value="#s.site" var="a" status="sta">
              <a href="/${oname }/page/pageconfig_app.action?siteid=${a.id}&stype=${a.type}&lightType=3&appId=10">
                <span title="编辑" class="glyphicon glyphicon-edit"></span>
              </a>
              <i class="split">|</i>
            </s:iterator>
            <s:if test='#s.istemplate!="Y"'>
              <a href="javascript:void(0)" onclick="delete_site('${s.id}')">
