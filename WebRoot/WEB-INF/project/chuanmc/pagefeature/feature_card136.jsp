...
<div class="popup-header">
  ...
  <h4 class="modal-title" id="myModalLabel">互动模块编辑-大转盘</h4>
  <!-- 
  <button type="button" class="btn btn-primary" onclick="newDZP()">新增</button>
   -->
</div>
...
<form enctype="multipart/form-data" method="post">
  ...
  <div class="tab-content">
	...
	<p>请选择需要抽奖的选项：(请至微互动模块处添加大转盘)</p>
	  <s:iterator value="dto.lotteryList" var="apt">
           <label class="block_label"><input type="radio" value="${apt.id}" name="dto.lotteryid" <s:if test="dto.lotteryid == #apt.id"> checked="checked" </s:if> /> ${apt.name}<a href="/${oname }/interact/edit_zhuanpan.action?lid=${apt.id }&mid=10003&omid=${apt.omid }">编辑</a> </label>
      </s:iterator>
     </div>
