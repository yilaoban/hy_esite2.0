...
<form  id="myform">
	...
	请选择需要投票的选项：<br/>
	  <s:iterator value="dto.interactVote" var="apt" status="st">
           <label class="forradio"><input type="radio" value="${apt.id}" name="dto.voteid" <s:if test='dto.voteid ==0 && #st.index==0'>checked="checked"</s:if><s:if test="dto.voteid == #apt.id"> checked="checked" </s:if> /> ${apt.title} </label><br />
      </s:iterator>
	<input type="button" value="确定" id="save123"/>
