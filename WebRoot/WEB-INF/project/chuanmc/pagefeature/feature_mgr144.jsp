...
<form  id="myform">
	<input type="hidden" name="dto.fid" value="${fid}" />
	  <s:iterator value="dto.renQiList" var="rq">
           <label class="forradio"><input type="radio" value="${rq.id}" name="dto.rqid" <s:if test="dto.rqid == #rq.id"> checked="checked" </s:if> /> ${rq.title} </label><br />
      </s:iterator>
	<input type="button" id="save144" value="确定"/>
	<input type="button" value="取消" onclick="closeFrame()"/>
</form>
<script type="text/javascript">
		$("#save144").click(function(){
			$.ajax({
		        cache: true,
		        type: "POST",
