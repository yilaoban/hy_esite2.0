...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('#wyycontent').html("");
		$('#service').html("");
		$.post("/${oname}/user/findYuYueServiceById.action",{"catid":catid,"serviceid":serviceid},function(data){
			if(data.yuYueService){
				var html = '<img src="'+data.yuYueService.img+'">'+
							'<p class="details_1">'+data.yuYueService.hydesc+'</p>'+
							'<p class="yuyue"><a href="/${oname}/user/show/'+data.current.xpageid+'/pcn/'+$('#hy_kv').val()+'.html">立即预约</a></p>';
				
				$('#service').append(html);
				$('#wyycontent').html(data.yuYueService.hyldesc);
			}
		});
