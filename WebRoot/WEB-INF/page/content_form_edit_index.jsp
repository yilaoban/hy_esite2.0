...
function changeLBS(id){
	$("#lbs_show_"+id).toggle();
	$("#lbs_hide_"+id).toggle();
}
</script>
<div class="wrap_content left_module">
	 <form action="content_form_edit_sub.action" method="post" class="formview" id="form1">
	 	<input name="record.formid" value="${formid }" type="hidden"/>
	 	<input name="record.id" value="${recordid }" type="hidden"/>
	 	<input name="ccid" value="${dto.current.id }" type="hidden"/>
	 	 <div class="toolbar pb10">
	  	<ul class="c_switch">
		  <li class="selected"><a href="">编辑内容</a></li>
		  </ul>
		...
		<s:iterator value="dto.colum" var="p">
				...
				<dl>
					<dt>${p.name }</dt>
					<dd>
						<input type="text" name="record.${p.mapping }" value="${p.record }"/>
					</dd>
				</dl>
				</s:if>
				...
				<dl>
					...
					<!-- 
					 ...
					 -->
					 <input type="text" class="text-medium" name="record.${p.mapping}" value="${p.record}" id="${p.mapping}"  onclick="changeContent('${p.mapping}')">
					</dd>
				</dl>
				</s:elseif>
				...
				<dl>
					<dt>${p.name }</dt>
					<dd>
						<script type="text/plain" id="myEditor${p.mapping}" name="record.${p.mapping }" style="width:100%;height:200px;">${p.record}</script>
					</dd>
				</dl>
				<script type="text/javascript">
				...
				<dl style="display: none" id="lbs_hide_${p.id }">
					<dt>${p.name }</dt>
					<dd>
						...
						市:<select class="lbs_city" id="city${p.mapping }">
							...
							<br><br>
						具体地址:<input type="text" class="qstr" id="qstr${p.mapping }" style="width:600px"/><br>
						<div>
				...
				<dl id="lbs_show_${p.id }">
					<dt>${p.name }</dt>
					<dd>
						<textarea rows="" cols="" readonly="readonly" style="width: 250px; height: 40px;">${p.record }</textarea>
						<a href="javascript:changeLBS('${p.id }')">换地址</a>
					</dd>
				</dl>
				</s:elseif>
				...
				<dl>
					...
					<dd class="clearfix">
						<div id="as${p.id }"></div>
						<div class="imgpicker" id="picker${p.id }">选择图片</div>
						<script type="text/javascript">
				...
				<dl>
					...
					<dd>
						配置文件地址：
						<input name="record.${p.mapping }" id="record${p.id }" value="${p.record }" style="width:500px;"/>
						<a id="view_${p.id}" href="${pageDomain }/${oname}/user/360show.action?p=${p.record}" target="_blank">预览</a>
						<div id="as${p.id }"></div>
						...
						$('#as${p.id }').diyUpload({
							...
							success:function( data ) {
								if(data.xmlPath!=""){
									...
									$("#view_${p.id}").attr("href"," ${pageDomain }/${oname}/user/360show.action?p="+data.xmlPath);
								}
								$("#picker${p.id }").hide();
							},
							error:function( err ) {
			...
			<dl>
				...
				<dd>
					...
					<input type="button" class="btn btn-primary" value="关闭" onclick="window.location='/${oname }/content/content_form_index.action?ccid=${dto.current.id}'"/>
				</dd>
			</dl>
	 </form>
</div>