...
<SCRIPT type="text/javascript">
		...
		function onClick(e, treeId, treeNode){
			var typeid = ${dto.current.typeid};
			window.location.replace("/"+'${oname}'+"/content/content_index.action?ccid="+treeNode.id+"&typeid="+typeid)
		}
			...
			$("#cmmSub").click(function(){
				 ...
				 $("#batchDel").click(function(){
				  ...
				  $("#batchUpload").click(function(){
						var srcString = 'upload_img.action?ccid='+${dto.current.id}+"&typeid="+${dto.current.typeid};
						var title="页面上传";
						layer.open({
								type : 2,
								area : ['500px','420px'],
		...
		<div class="btn-group">
			  <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" >批量操作<span class="caret"></span></button>
			  	<ul class="dropdown-menu">
			    	...
			    	<s:if test='dto.current.type=="T" && (oname=="xujie"||oname=="xuantu" || oname=="adBlueFocus")'>
			...
			<s:iterator value="dto.product" var="t" status="st">
					<tr>
						...
						<td>
							...
							<a href="javascript:void(0);" onclick="up_T('${t.id }','${t.catid }','${dto.current.type }','${st.count }')">上移</a><i class="split">|</i>
							<a href="javascript:void(0);" onclick="down_T('${t.id }','${t.catid }','${dto.current.type }','${st.count }')">下移</a><i class="split">|</i>
		...
		<!-- 图片 -->
			<s:iterator value="dto.pictureList" var="t" status="st">
					<tr>
						...
						<td>
							<input id="idx${st.count }" value="${t.idx }" type="hidden"/>
							<a href="/${oname}/content/edit_picture_pre.action?contentId=${t.id }">编辑</a><i class="split">|</i>
							<a href="javascript:void(0);" onclick="up_T('${t.id }','${t.catid }','${dto.current.type }','${st.count }')">上移</a><i class="split">|</i>
							<a href="javascript:void(0);" onclick="down_T('${t.id }','${t.catid }','${dto.current.type }','${st.count }')">下移</a><i class="split">|</i>
			...
			<s:iterator value="dto.newList" var="t" status="st">
					<tr>
						...
						<td>
							...
							<a href="/${oname}/content/edit_new_pre.action?contentId=${t.id }">编辑</a><i class="split">|</i>
							<a href="javascript:void(0);" onclick="up_T('${t.id }','${t.catid }','${dto.current.type }', '${st.count }')">上移</a><i class="split">|</i>
							<a href="javascript:void(0);" onclick="down_T('${t.id }','${t.catid }','${dto.current.type }','${st.count }')">下移</a><i class="split">|</i>
							<s:if test="dto.topicList.contains(#t.id)">
								已开通
		...
		<!-- 视频 -->
			<s:iterator value="dto.videoList" var="t" status="st">
					<tr>
						...
						<td>
							<input id="idx${st.count }" value="${t.idx }" type="hidden"/>
							<a href="/${oname}/content/edit_video_pre.action?contentId=${t.id }">编辑</a><i class="split">|</i>
							<a href="javascript:void(0);" onclick="up_T('${t.id }','${t.catid }','${dto.current.type }','${st.count }')">上移</a><i class="split">|</i>
							<a href="javascript:void(0);" onclick="down_T('${t.id }','${t.catid }','${dto.current.type }','${st.count }')">下移</a><i class="split">|</i>
