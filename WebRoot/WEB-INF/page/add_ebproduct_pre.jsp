...
<form action="add_ebproduct_sub.action" method="post" enctype="multipart/form-data" class="formview jNice" id="form1">
<span id="errrSpan" style="color:red"><strong style="color:red"><s:fielderror theme="" fieldName="error"/></strong> </span>
  <div class="toolbar pb10">
	  ...
	  <a href="/${oname }/content/ebProductList.action?ccid=${ccid }&subtype=${subtype}" class="return" title="返回"></a>
	</div>
	
	<input type="hidden" name="productType" value="${subtype }"/>
 	<dl>
	 	<dt>分类</dt>
		<dd>
	...
	<s:if test='subtype == "W"'>
		<dl>
		 	<dt>最多抵扣的积分</dt>
			<dd>
				<input type="text" class="text-medium" name="maxjf" onkeyup="this.value = this.value.replace(/\D/g,'');">
			</dd>
		</dl>
	</s:if>
	<dl>
	 	<dt>每人最多购买次数</dt>
		<dd>
			<input type="num" class="text-medium" name="personlimit" value="0">（0表示不限制购买）
		</dd>
	</dl>
	<dl>
	 	<dt>产品总数</dt>
	...
	<dl>
	 	<dt>商品类型</dt>
		<dd>
			<label class="forradio"><input type="radio" name="subtype" value="S"  checked="checked">物流商品</label>
			<label class="forradio"><input type="radio" name="subtype" value="K">自提商品</label>
		</dd>
	</dl>
	<dl>
	 	<dt>摘要</dt>
